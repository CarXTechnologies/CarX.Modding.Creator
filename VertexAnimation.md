...
4. Export as `Binary` or `Wavefront`. Rebuild both MapUploader and the client with the updated Creator module. Older clients cannot display animation resources.
...
- GPU instancing shares identical prototypes; this is not MassRenderer's URP-specific multi-draw-indirect implementation. The current client adapter uses Entities Graphics / RenderCore; this renderer is not part of the file-format identity.
